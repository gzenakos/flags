...
function clearBoard(){
    ...
    document.querySelector("#btn-next-round").setAttribute('disabled','true');
    if(!document.querySelector('#success').classList.contains('hidden')){
        document.querySelector('#success').classList.add('hidden');
    }
    if(!document.querySelector('#fail').classList.contains('hidden')){
        document.querySelector('#fail').classList.add('hidden');
    }
    document.querySelector('#progress').setAttribute("value", '');
            ...
            if (reply.lenth < 1) {
                ...
                if (realNeighboursLength < 1){
                    ...
                    for (let i = 0; i < (realNeighboursLength); i++) {                                  //REAL NEIGHBOURS
                        ...
                        neighboursFakeAndREal.push(neighbourReal[0]);                                   //PUSH TO REAL & FAKE NEIGHBOURS
                        RealNeighbourCodes2.push(neighbourReal[0].code);
                        countryObjectsFiltered = arrayRemove(countryObjectsFiltered, neighbourReal[0]); //remove selected country in order not to add it as fake
                    }

                    for (let i = 0; i < (realNeighboursLength); i++) {                                  //FAKE NEIGHBOURS
                        var neighbourFake =  shuffleArray(countryObjectsFiltered)[0];
                        neighboursFakeAndREal.push(neighbourFake);                                      //PUSH TO REAL & FAKE NEIGHBOURS
                        countryObjectsFiltered = arrayRemove(countryObjectsFiltered, neighbourFake[0]);  //remove fake country in order not to add it more tha once
                    }
                        neighboursFakeAndREalArray = Object.values(neighboursFakeAndREal)             // MAKE IT ARRAY
                        var shuffledNeighboursFakeAndReal = shuffleArray(neighboursFakeAndREalArray); // SHUFFLE DATA
